...
```text
Yes. Good first issues are kept small and reviewable. The current one is #80,
which asks for a concise README first-run troubleshooting link near the
release-tag install section. For adapter work, the most useful first step is an
integration request describing the framework and failure state you need to
debug.
```
