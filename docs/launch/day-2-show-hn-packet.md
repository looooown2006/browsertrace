...
```text
Yes. Good first issues are kept small and reviewable. The current one is #130,
which asks for a concise `docs/llms.txt` troubleshooting snippet using the JSON
CLI commands. For adapter work, the most useful first step is an integration
