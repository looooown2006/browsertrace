...
```text
Yes. Good first issues are kept small and reviewable. The current one is #96,
which asks for a concise README healthy-doctor-output note near the release-tag
install section. For adapter work, the most useful first step is an integration
