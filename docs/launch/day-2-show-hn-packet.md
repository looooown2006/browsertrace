...
```text
Yes. Good first issues are kept small and reviewable. The current one is #91,
which asks for a concise README first-run feedback issue note near the release-tag
install section. For adapter work, the most useful first step is an integration request
