...
```text
Yes. Good first issues are kept small and reviewable. The current one is #111,
which asks for a concise README public-safe export privacy note near the release-tag install
checks. For adapter work, the most useful first step is an integration request
