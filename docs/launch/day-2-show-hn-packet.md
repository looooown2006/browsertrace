...
```text
Yes. Good first issues are kept small and reviewable. The current one is #131,
which asks for a concise README link to the `docs/llms.txt` troubleshooting
context. For adapter work, the most useful first step is an integration request
