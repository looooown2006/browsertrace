...
The document must include the canonical repo URL `https://github.com/aaronlab/browsertrace`, the current release `v0.1.11`, and the current star count audit command:
...
```bash
rg -n "aaronlab/browsertrace|v0.1.11|Show HN|Product Hunt|Owner-Only|Daily Metrics" LAUNCH.md docs/launch
...
```bash
gh release view v0.1.11 --json tagName,name,isDraft,isPrerelease,assets,url
```

Expected: `v0.1.11` exists and includes wheel, sdist, and demo HTML assets.
